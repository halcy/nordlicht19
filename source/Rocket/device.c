...
static inline int socket_poll(SOCKET socket)
{
	...
	FD_ZERO(&fds);
	FD_SET(socket, &fds);
    int ret = my_select(socket);
    return(ret > 0);
}

int xsend(SOCKET s, const void *buf, size_t len, int flags) {
	int ret = send(s, (const char *)buf, len, flags);
    return ret != (int)len;
}

int xrecv(SOCKET s, void *buf, size_t len, int flags) {
